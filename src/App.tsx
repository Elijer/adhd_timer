  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-sand-100 py-20 px-4">
      <div className="text-center border-1 border-sand-700/20 p-16">
        ...
        <h1 className="text-sm text-sand-700/90 mb-16">
          For those on adhd time
        </h1>
...
function AuthenticatedApp({ user }: { user: any }) {
  ...
  useEffect(() => {
    ...
    if (!(activeTimerId in localTimes)) {
      setLocalTimes((prev) => ({
        ...prev,
        [activeTimerId]: activeTodo.time as number,
      }));
    }
  ...
  const totalSeconds = todos.reduce(
    (sum: number, todo: any) => sum + todo.time,
    0
  );
  ...
  const handleToggleTimer = (todo: any) => {
    if (activeTimerId === todo.id) {
      ...
      db.transact(db.tx.todos[todo.id].update({ time: currentTimeInSeconds }));
      setActiveTimerId(null);
    } else {
        ...
        <div className="w-full max-w-3xl">
          ...
          <div className="text-center">
            <div className="text-6xl text-sand font-bold mb-3">
              <span className="border-b-1 border-sand-700/20">
          ...
          <form
            ...
            className="flex text-3xl sm:text-4xl mb-12 sm:mb-18 mt-12 sm:mt-16"
          >
              ...
              <Droppable droppableId="droppable">
                {(droppableProvided) => (
                  <div
                    ...
                    className={`${
                      todos && todos.length > 0 ? "border-b-1 border-sand" : ""
                    }`}
                  >
                              ...
                              <div className="flex items-center justify-end w-full sm:w-auto gap-3 sm:gap-4 mt-1 sm:mt-0">
                                <span
                                  className={`w-20 sm:w-24 text-right ${
                                    (localTimes[todo.id] ?? todo.time) === 0
